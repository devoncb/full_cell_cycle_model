...
start_phase_pop_cap = 2000; %maximum number of cells during steady state phase
...
p1_syn_G2 = 1;              %AU/min constant portion of synthesis rate
p1_syn_rate_G2 = 0;          %AU/(min*fl) scaling portion of synthesis rate
p1_syn_G1 = 1;              %AU/min constant portion of synthesis rate
p1_syn_rate_G1 = 0;          %AU/(min*fl) scaling portion of synthesis rate
p2_syn_G2 = 0;              %AU/min constant portion of synthesis rate
p2_syn_rate_G2 = 1/5e4;          %AU/(min*fl) scaling portion of synthesis rate
p2_syn_G1 = 0;              %AU/min constant portion of synthesis rate
p2_syn_rate_G1 = 1/5e4;          %AU/(min*fl) scaling portion of synthesis rate
% partition1 = .6;          %partitioning behavior, 0 means proportional, 1 fixed
% partition2 = .6;          %partitioning behavior, 0 means proportional, 1 fixed
kp = 1.441;
...
%                         cell_p2(k,i) * .5; %allot protein to daughter
                    ...
                    cell_p1(new_cell_num,i+1) = pt1*kp*vd/vm/(1+kp*vd/vm); %allot protein to daughter
                    cell_p1(k,i+1) =  pt1 - cell_p1(new_cell_num,i+1); %allot protein to mother
                    cell_p2(new_cell_num,i+1) = pt2*kp*vd/vm/(1+kp*vd/vm); %allot protein to daughter
                    cell_p2(k,i+1) =  pt2 - cell_p2(new_cell_num,i+1); %allot protein to mother
...
if sim_stage == 2
    save('20180719 Size-Partitioning Simulation Results WHI5 Partitioning Constitutive.mat')
end
...
%%
%Protein Plots
figure('Name','Protein in G1')
hold on
[inc_mean_y,inc_mean_x] = KS_bindata_mean_20141016(G1_amount_volume,...
    G1_amount_p1./G1_amount_volume, 15);
[inc_err_y,inc_err_x] = KS_bindata_stderr_20141016(G1_amount_volume,...
    G1_amount_p1./G1_amount_volume, 15);
shadedErrorBar(inc_mean_x, inc_mean_y, inc_err_y, 'r',.5)
[orig_mean_y,orig_mean_x] = KS_bindata_mean_20141016(G1_amount_volume,...
    G1_amount_p2./G1_amount_volume, 15);
[orig_err_y,orig_err_x] = KS_bindata_stderr_20141016(G1_amount_volume,...
    G1_amount_p2./G1_amount_volume, 15);
shadedErrorBar(orig_mean_x, orig_mean_y, orig_err_y, 'b',.5)
axis([0 inf 0 inf])
xlabel('G1 Volume')
ylabel('G1 Concentration')
hold off
...
%% export protein at birth v volume at birth
...
writetable(table(final_raw_values),'temp_excel.xlsx','Sheet',1)
